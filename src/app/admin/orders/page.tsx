...
const AdminOrdersPage = () => {
  ...
  const [quickDateFilter, setQuickDateFilter] = useState<'all' | 'daily' | 'weekly' | 'monthly' | 'yearly'>('all');
  ...
  const applyQuickDateFilter = (filter: 'all' | 'daily' | 'weekly' | 'monthly' | 'yearly') => {
    const now = new Date();
    ...
    switch (filter) {
      ...
      case 'monthly':
        ...
        setDateRange({ from: monthStart, to: now });
        break;
      case 'yearly':
  ...
  const handleBulkPrint = () => {
    ...
    if (selectedOrderIds.length === 0) {
      toast({ title: "تحذير", description: "الرجاء اختيار طلبات للطباعة", variant: "destructive" });
      return;
    }

    // Create URL with all selected order IDs
    const idsParam = selectedOrderIds.join(',');
    window.open(`/admin/orders/print?ids=${idsParam}`, '_blank', 'height=842,width=595,resizable=yes,scrollbars=yes');

    toast({
      title: "تم فتح الطباعة",
      description: `جاري طباعة ${selectedOrderIds.length} بوليصة`
    });
  }
        ...
        <Card className="glass-card border-none">
          <CardHeader>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-4 border-t">
              ...
              <Button
                variant={quickDateFilter === 'daily' ? 'default' : 'outline'}
                size="sm"
                onClick={() => applyQuickDateFilter('daily')}
              >
                يومي
              </Button>
              <Button
                variant={quickDateFilter === 'weekly' ? 'default' : 'outline'}
                size="sm"
                onClick={() => applyQuickDateFilter('weekly')}
              >
                أسبوعي
              </Button>
              <Button
                variant={quickDateFilter === 'monthly' ? 'default' : 'outline'}
                size="sm"
                onClick={() => applyQuickDateFilter('monthly')}
              >
                شهري
              </Button>
              <Button
                variant={quickDateFilter === 'yearly' ? 'default' : 'outline'}
                size="sm"
                onClick={() => applyQuickDateFilter('yearly')}
              >
                سنوي
              </Button>
            </div>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    ...
                    <DropdownMenuItem onSelect={handleBulkPrint}>
                      <Printer className="ml-2 h-4 w-4" /> طباعة البوليصات
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
